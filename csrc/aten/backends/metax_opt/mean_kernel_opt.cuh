...
#include "metax_elementwise_opt.cuh"
...
__global__ void MeanAlongDimKernel(
    int64_t outer,
    int64_t reduce,
    int64_t inner,
  ...
  out[outer_idx * inner + inner_idx] = static_cast<out_t>(sum * inv_reduce);
}
...
at::Tensor MeanAlongDim(
  ...
  MeanAlongDimKernel<scalar_t, acc_t, out_t>
  ...
  TORCH_CHECK(
      err == cudaSuccess,
      "MetaX mean kernel launch failed: ",
...
inline at::Tensor MeanDimKernelOpt(
